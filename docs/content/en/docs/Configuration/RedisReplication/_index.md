...
Redis replication configuration can be customized by [values.yaml](https://github.com/elrondwong/helm-charts/blob/main/charts/redis-replication/values.yaml). The recommended way of managing the setup is using `helm` but if the setup is not maintained by it, `YAML` CRD parameters can be modified in the manifest.
